...
def run_osm2world(input_path, output_path, scale, exclude_buildings):
    # Use environment variable or default path for OSM2World JAR
...
def run_blender(obj_path, bounds, args):
    # Use system Blender (installed in Docker or locally)
    blender_path = os.environ.get('BLENDER_PATH', '/usr/local/bin/blender')

    # Fall back to checking if blender is in PATH
