...
def create_process_settings_config(optimize_print: bool = False) -> str:
    """
    Create Metadata/process_settings.config with print quality parameters.

    Bambu Studio reads this INI-style config for process settings when
    importing a 3MF project file.
    """
    preset = PRINT_SETTINGS['optimized' if optimize_print else 'standard']
    lines = ['; generated by WOSMap for Bambu Studio / OrcaSlicer']
    lines.append('; Preset: ' + ('Optimized (ironing + fine layers)' if optimize_print else 'Standard'))
    lines.append('')
...
def create_multicolor_3mf(stl_files: Dict[str, str], output_path: str, optimize_print: bool = False) -> str:
    ...
    with zipfile.ZipFile(output_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr('Metadata/process_settings.config', create_process_settings_config(optimize_print))
